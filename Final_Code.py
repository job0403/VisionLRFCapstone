...
OUTPUT_PATH = "output/output_with_depth.mp4"
SEGMENT_MODEL = "weights/yolov8_rail_seg.pt"

SAVE_OUTPUT = False                 # <- toggle saving
OUTPUT_CODEC = 'mp4v'

FPS_LOG_PATH = "fps_log"        # <-- Path for the FPS log file

DETECT_MODEL = "weights/yolo11n.pt"
...
def get_current_min_distance():
    ...
    if ai_closest_distance != float('inf'):
        distances.append(ai_closest_distance)
...
# ----------------------------
# Setup FPS Logging File
# ----------------------------
fps_file = open(FPS_LOG_PATH, "w")        
fps_file.write("FPS\n")                   # <-- ADDED: Just the FPS header
...
for av_frame in container.decode(stream):
            ...
            if xc2 > xc1:
                ...
                if lx2 > lx1 and ly2 > ly1:
                    ...
                    if patch.size:
                        ...
                        ai_closest_distance = min(ai_closest_distance, closest)
    ...
    # <-- ADDED: Log ONLY the FPS to the text file
...
container.close()
fps_file.close()                          # <-- ADDED: Close the text file safely
if SAVE_OUTPUT and out_writer is not None:
    out_writer.release()
